...
const Page = () => {
  ...
  const handleSirenSubmit = (sirenData = null) => {
    console.log('Received SIREN data:', sirenData);
    setCustomerData(sirenData);
    setSirenModalOpen(false);
    setCustomerModalOpen(true);
  };

  const openCustomerModal = (client = null) => {
    if (client) {
      // Mode mise à jour avec les données du client
      setCustomerData(client);
      setIsUpdating(true);
    } else {
      // Mode insertion, sans données initiales
  ...
  const onAddClient = (client, isUpdating) => {
    if (isUpdating) {
        ...
        <header className="flex justify-between items-center">
          <div className="flex justify-center items-center gap-12">
            ...
            <div>
              ...
              <SirenModal isOpen={isSirenModalOpen} onClose={() => setSirenModalOpen(false)} onSirenSubmit={handleSirenSubmit} />
              <CustomerModal
            ...
            {clients.map((client) => (
                <tr key={client.id} className="bg-[#e7effc]">
                  ...
                  <td className="p-3 text-center">
                    <DropdownMenu open={openDropdownId === client.id} onOpenChange={() => toggleDropdown(client.id)}>
